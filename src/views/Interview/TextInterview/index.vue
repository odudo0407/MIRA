<!-- src/views/Interview/TextInterview/index.vue -->
<template>
        ...
        <div class="main-content-area">
          ...
          <div class="chat-container">
            <div class="messages-list" ref="messagesContainer">
              <!-- 欢迎消息 -->
              <div v-if="messages.length === 0 && interviewStarted" class="message-item ai">
                <div class="message-avatar">🤖</div>
                <div class="message-bubble">
                  <div class="message-sender">AI面试官</div>
              ...
              >
                ...
                <div class="message-bubble">
                  <div class="message-sender">{{ msg.type === 'ai' ? 'AI面试官' : '我' }}</div>
            ...
            <div class="input-area">
              <textarea 
                ...
                class="answer-input"
                :placeholder="interviewStarted ? '输入你的回答...' : '请先开始面试'"
                :disabled="!interviewStarted || aiTyping"
                @keydown.enter.prevent="sendMessage"
              ></textarea>
              <button 
                class="send-btn" 
                @click="sendMessage"
                :disabled="!interviewStarted || !currentAnswer.trim() || aiTyping"
              >
...
const welcomeMessage = computed(() => {
  const messages = {
    java: '你好，我是你的Java面试官。今天我们将考察Java基础、Spring Boot和微服务相关知识。我们先从Java基础开始吧。',
    frontend: '你好，我是你的前端面试官。今天我们将考察Vue/React、JavaScript和浏览器原理。让我们开始第一个问题。',
    python: '你好，我是你的Python面试官。今天我们将考察Python基础、数据结构和算法。准备好就开始吧。'
  }
  return messages[jobId.value] || '你好，我是你的面试官，请准备好开始面试。'
})
...
const sendMessage = async () => {
  if (!currentAnswer.value.trim() || !interviewStarted.value || aiTyping.value) return
  ...
  // TODO: 接后端后，这里应该调用API发送回答并获取下一个问题
  // 减少思考时间
  setTimeout(() => {
    aiTyping.value = false
    
    // 检查是否还有问题
    if (messages.value.filter(m => m.type === 'ai').length < questionCount.value) {
      askNextQuestion()
    } else {
      // 面试结束
      endInterview()
...
const askNextQuestion = () => {
  const base = baseQuestionBank[jobId.value] || baseQuestionBank.frontend
  const extra = extraQuestions[jobId.value] || extraQuestions.frontend
  const allQuestions = questionCount.value === 11 ? [...base, ...extra] : base
  
  const askedCount = messages.value.filter(m => m.type === 'ai').length
  
  if (askedCount < allQuestions.length) {
    const aiMessage = {
      type: 'ai',
      content: allQuestions[askedCount],
      time: currentTime.value
    }
...
const endInterview = () => {
  ...
  const interviewRecord = {
    ...
    topic: `${jobName.value}文字面试`,
    duration: elapsedSeconds.value,
    rounds: messages.value.filter(m => m.type === 'user').length,
    score: 4.0,
    tags: [jobName.value, '文字面试'],
    strengths: ['表达清晰', '逻辑性强'],
    weaknesses: ['回答可以更详细一些'],
